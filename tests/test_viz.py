"""Tests for the plotnine visualization backend (`greenwood.viz.plotnine`).

These need plotnine (the `plotnine` extra); the module is skipped when it is absent. Rendering
...
def test_plot_survival_draws_without_error(km_grouped: gw.KaplanMeier) -> None:
    gw.viz.plotnine.plot_survival(km_grouped).draw(show=False)
...
def test_plot_survival_layers(km_overall: gw.KaplanMeier) -> None:
    ...
    plot = gw.viz.plotnine.plot_survival(km_overall, conf_int=True, censor_marks=True)
...
def test_plot_survival_without_ci_has_no_ribbon(km_overall: gw.KaplanMeier) -> None:
    plot = gw.viz.plotnine.plot_survival(km_overall, conf_int=False)
    geoms = {type(layer.geom).__name__ for layer in plot.layers}
...
def test_risk_table_composition_stacks(km_grouped: gw.KaplanMeier) -> None:
    ...
    comp = gw.viz.plotnine.plot_survival(km_grouped, risk_table=True)
