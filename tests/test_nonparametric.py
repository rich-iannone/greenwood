...
class TestNelsonAalenCI:
    def test_ci_bracket_log(self) -> None:
        ...
        assert (df["estimate"] <= df["conf_high"] + 1e-12).all()
    ...
    def test_log_and_plain_ci_differ(self) -> None:
        ...
        assert not np.allclose(df_log["conf_low"].values, df_plain["conf_low"].values)

    def test_invalid_conf_type_raises(self) -> None:
        with pytest.raises(ValueError, match="conf_type"):
            NelsonAalen(conf_type="bogus")

    def test_invalid_conf_level_raises(self) -> None:
        with pytest.raises(ValueError, match="conf_level"):
            NelsonAalen(conf_level=0.0)
...
class TestNelsonAalenWeighted:
    ...
    def test_double_weights_preserve_cumhaz(self) -> None:
        ...
        np.testing.assert_allclose(na2.cumhaz_, na1.cumhaz_)
