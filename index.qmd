...
2. **Estimate and summarize**: use `KaplanMeier`, `NelsonAalen`, or `Turnbull` (for interval-censored data) to describe the data, then compare groups with `logrank_test`, trend tests, or RMST comparisons.
...
**Descriptive statistics**: Kaplan-Meier curves with Greenwood confidence intervals, restricted mean survival time (RMST), quantiles, and medians. Nelson-Aalen cumulative hazard. Turnbull's NPMLE for interval-censored data. Event tables and risk sets.
...
- **Installation**: get started with `pip install greenwood` or see [Installation](user-guide/installation.qmd).
- **Quick tour**: new to Greenwood? Start with the [Quick start](user-guide/quick-start.qmd) for a fast hands-on tour (5 min read).
- **Learn survival analysis**: new to survival analysis? Begin with [Survival data and the Surv object](user-guide/survival-data.qmd) to understand censoring.
- **Build models**: jump to [Cox regression](user-guide/cox-regression.qmd) or [Parametric models](user-guide/parametric-models.qmd).
